...
echo "Starting Postgres..." && docker compose up -d postgres && sleep 2
echo "Starting Redis..." && docker compose up -d redis && sleep 2
echo "Starting Airflow Init..." && docker compose up -d airflow-init && sleep 10
echo "Starting Airflow Webserver..." && docker compose up -d airflow-webserver && sleep 4
echo "Starting Airflow Scheduler..." && docker compose up -d airflow-scheduler && sleep 4
echo "Starting Airflow Worker..." && docker compose up -d airflow-worker && sleep 4
echo "Starting Airflow Triggerer..." && docker compose up -d airflow-triggerer && sleep 4
echo "Starting Airflow CLI..." && docker compose up -d airflow-cli && sleep 3
echo "Starting Flower..." && docker compose up -d flower && sleep 3
